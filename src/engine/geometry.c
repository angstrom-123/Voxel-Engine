...
static vec2 _unpack(uint32_t packed)
{
                              ...
                              uint32_t *indices, vertex_t *vertices)
{
    ...
    {
        vec2 uv = _unpack(verts[i].uv);
        uv = em_add_vec2(uv_lookup[(desc->type * 6) + desc->face], uv);

        verts[i].uv = PACK(uv.elements[0], uv.elements[1]);
        verts[i].x += (desc->x);
        verts[i].y += (desc->y);
        verts[i].z += (desc->z);
    }
                                ...
                                chunk_data_t *s, chunk_data_t *w)
{
    ...
    if (!blocks)
    {
        ENGINE_LOG_ERROR("Failed to mesh chunk because block data is NULL\n", NULL);
        exit(1);
    }
    uint8_t *ptr = &blocks->types[0][0][0];
