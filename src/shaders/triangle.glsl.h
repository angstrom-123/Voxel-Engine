...
/*
    ...
    Cmdline:
        sokol-shdc -i ./src/shaders/triangle.glsl -o ./src/shaders/triangle.glsl.h -l glsl300es

    Overview:
    =========
    Shader program: 'triangle':
        Get shader desc: tri_triangle_shader_desc(sg_query_backend());
        Vertex Shader: vs
        Fragment Shader: fs
        Attributes:
            ATTR_tri_triangle_position => 0
            ATTR_tri_triangle_color0 => 1
    Bindings:
*/
...
#endif
#endif
#define ATTR_tri_triangle_position (0)
#define ATTR_tri_triangle_color0 (1)
/*
    #version 300 es
...
*/
static const uint8_t tri_vs_source_glsl300es[176] = {
...
*/
static const uint8_t tri_fs_source_glsl300es[175] = {
    0x23,0x76,0x65,0x72,0x73,0x69,0x6f,0x6e,0x20,0x33,0x30,0x30,0x20,0x65,0x73,0x0a,
...
static inline const sg_shader_desc* tri_triangle_shader_desc(sg_backend backend) {
    if (backend == SG_BACKEND_GLES3) {
        static sg_shader_desc desc;
        static bool valid;
        if (!valid) {
            valid = true;
            desc.vertex_func.source = (const char*)tri_vs_source_glsl300es;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = (const char*)tri_fs_source_glsl300es;
            desc.fragment_func.entry = "main";
            desc.attrs[0].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[0].glsl_name = "position";
            desc.attrs[1].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[1].glsl_name = "color0";
            desc.label = "tri_triangle_shader";
        }
        return &desc;
    }
    return 0;
